...
'''
 ...
 Info
     Defined class SignalHandler with attribute(s) and method(s).
...
try:
    from gen_signal.pro.config import ProConfig
    from gen_signal.pro.config.pro_name import ProName
    from gen_signal.pro.read_template import ReadTemplate
    from gen_signal.pro.write_template import WriteTemplate
...
class SignalHandler(FileChecking, ProConfig, ProName):
    '''
        Defined class SignalHandler with attribute(s) and method(s).
        Generate module file generator_test.py by template and parameters.
        It defines:
            ...
            :methods:
                ...
                | __str__ - dunder method for SignalHandler.
    '''

    GEN_VERBOSE = 'GEN_SIGNAL::PRO::SIGNAL_HANDLER'
    ...
    def __init__(self, verbose=False):
        ...
        verbose_message(SignalHandler.GEN_VERBOSE, verbose, 'init generator')
        self.__reader = ReadTemplate(verbose=verbose)
        self.__writer = WriteTemplate(verbose=verbose)
        project_structure = '{0}{1}'.format(
            dirname(realpath(__file__)), SignalHandler.PRO_STRUCTURE
        )
    ...
    def gen_setup(self, pro_name, verbose=False):
        ...
        verbose_message(
            SignalHandler.GEN_VERBOSE, verbose,
            'generating signal handler', pro_name
        )
        if bool(self.config):
            setup_content = self.__reader.read(self.config, verbose=verbose)
            if setup_content:
    ...
    def __str__(self):
        '''
            Dunder method for SignalHandler.
